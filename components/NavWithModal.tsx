...
function NavA({ onOpenModal }: { onOpenModal: () => void }) {
  return (
    <nav className="sticky top-0 z-50 bg-brand-black text-white border-b-2 border-brand-black">
      <div className="flex items-center justify-between px-10 max-md:px-6 h-12">
        ...
        <div className="flex items-center gap-6">
          <Link href="/login" className="text-gray-400 hover:text-white text-xs font-mono uppercase tracking-wide transition-colors max-md:hidden">Log in</Link>
...
/* ─── Theme B ─── Slim dark nav, warm accent ─── */
function NavB({ onOpenModal }: { onOpenModal: () => void }) {
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-tb-dark h-[44px] flex items-stretch px-4">
      ...
      <div className="hidden lg:flex items-stretch gap-0">
        <Link href="/#how-it-works" className="flex items-center px-4 font-mono text-[13px] uppercase tracking-[0.06em] text-white/80 hover:text-white transition-opacity">
          How It Works
        </Link>
        ...
        <Link href="/about" className="flex items-center px-4 font-mono text-[13px] uppercase tracking-[0.06em] text-white/80 hover:text-white transition-opacity">About</Link>
        <Link href="/pricing" className="flex items-center px-4 font-mono text-[13px] uppercase tracking-[0.06em] text-white/80 hover:text-white transition-opacity">Pricing</Link>
        <button onClick={onOpenModal} className="flex items-center px-5 bg-tb-primary text-white font-mono text-[13px] uppercase tracking-[0.08em] hover:bg-tb-cta-hover transition-colors">
          Meet Q
        </button>
      </div>
    </nav>
